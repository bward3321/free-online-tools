        ...
        <div className="max-w-[800px] mx-auto px-4 py-8 md:py-12">
          ...
          <article className="space-y-6 mb-10">
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">Markdown Table Syntax</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>Markdown tables use pipes (<code style={{ color: "#059669" }}>|</code>) to separate columns and hyphens (<code style={{ color: "#059669" }}>---</code>) to create the header separator row. The first row is always the header. Alignment is controlled with colons in the separator: <code style={{ color: "#059669" }}>:---</code> for left, <code style={{ color: "#059669" }}>:---:</code> for center, <code style={{ color: "#059669" }}>---:</code> for right. This syntax is supported across GitHub, GitLab, and most Markdown renderers.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">Why Use a Visual Table Builder?</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>Typing Markdown tables by hand is tedious and error-prone. Miscounting pipes, misaligning columns, and forgetting the separator row are common mistakes. The visual builder eliminates these issues &mdash; you type content directly into cells, and the tool generates correctly formatted Markdown. Add or remove rows and columns with a click.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">Tips for Better Tables</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>Keep header text short and descriptive. Use alignment to improve readability &mdash; center column headers, right-align numbers. For tables with many columns, consider splitting into multiple smaller tables. If you need merged cells or complex layouts, Markdown tables won&apos;t suffice &mdash; use raw HTML tables instead.</p>
            </section>
          </article>
          <div className="rounded-xl border p-4 mb-8 text-center" style={{ backgroundColor: "#0596691a", borderColor: "#05966940" }}>
            <p className="font-semibold" style={{ fontSize: "16px" }}>The table builder opens automatically below. Fill in your cells and copy the Markdown.</p>
          </div>
        </div>
      </div>
