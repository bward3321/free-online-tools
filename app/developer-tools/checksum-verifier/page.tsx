        ...
        <div className="max-w-[800px] mx-auto px-4 py-8 md:py-12">
          ...
          <article className="space-y-6 mb-10">
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">Step-by-Step File Verification</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}><strong style={{ color: "var(--text)" }}>Step 1:</strong> Download the file you want to verify. <strong style={{ color: "var(--text)" }}>Step 2:</strong> Find the official checksum on the publisher&apos;s website (usually on the download page or in a .sha256 file). <strong style={{ color: "var(--text)" }}>Step 3:</strong> Drop your file in the tool below. <strong style={{ color: "var(--text)" }}>Step 4:</strong> Paste the expected hash. <strong style={{ color: "var(--text)" }}>Step 5:</strong> Click Verify &mdash; the tool computes the file&apos;s hash and compares it instantly.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">Finding Official Checksums</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>Software publishers provide checksums in different ways. <strong style={{ color: "var(--text)" }}>Linux ISOs:</strong> Look for SHA256SUMS or SHA512SUMS files on download mirrors. <strong style={{ color: "var(--text)" }}>macOS/Windows apps:</strong> Check the &ldquo;Downloads&rdquo; or &ldquo;Releases&rdquo; page for hash values. <strong style={{ color: "var(--text)" }}>GitHub releases:</strong> Many projects include checksums in release notes or provide a CHECKSUMS file. <strong style={{ color: "var(--text)" }}>Package managers:</strong> npm, pip, and cargo verify checksums automatically.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">What a Mismatch Means</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>If the checksum doesn&apos;t match: <strong style={{ color: "var(--text)" }}>Most likely:</strong> The download was corrupted (try downloading again). <strong style={{ color: "var(--text)" }}>Possible:</strong> You&apos;re comparing against the wrong version or platform. <strong style={{ color: "var(--text)" }}>Rare but serious:</strong> The file has been tampered with. If re-downloading doesn&apos;t fix the mismatch and you&apos;re sure you have the correct expected hash, do not install the file and report the issue to the publisher.</p>
            </section>
          </article>
          <div className="rounded-xl border p-4 mb-8 text-center" style={{ backgroundColor: "#8BE9FD1a", borderColor: "#8BE9FD40" }}>
            <p className="font-semibold" style={{ fontSize: "16px" }}>Drop a file below and paste the expected checksum to verify.</p>
          </div>
        </div>
      </div>
