        ...
        <div className="max-w-[800px] mx-auto px-4 py-8 md:py-12">
          ...
          <article className="space-y-6 mb-10">
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">How File Checksums Work</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>A checksum is a cryptographic hash of a file&apos;s contents &mdash; a unique fingerprint that changes if even a single byte is modified. When you download software, the publisher provides a checksum (usually SHA-256). By computing the checksum of your downloaded file and comparing it to the official value, you can verify the file is <strong style={{ color: "var(--text)" }}>authentic and uncorrupted</strong>. This protects against corrupted downloads, man-in-the-middle attacks, and tampered files.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">Verifying Software Downloads</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>Most software publishers provide checksums on their download pages. Linux distributions publish SHA-256 sums for ISO images. Package managers (npm, pip, cargo) verify checksums automatically. When you download critical software &mdash; operating systems, security tools, cryptocurrency wallets &mdash; always verify the checksum before installing. Drop the file here, paste the official checksum, and get instant verification.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">100% Client-Side Processing</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>Your files <strong style={{ color: "var(--text)" }}>never leave your device</strong>. The tool reads files using the browser&apos;s File API and hashes them using the native Web Crypto API &mdash; the same cryptographic engine used by TLS for HTTPS connections. Files are processed in chunks, so even multi-gigabyte files work without running out of memory. A progress bar shows real-time hashing progress for large files.</p>
            </section>
          </article>
          <div className="rounded-xl border p-4 mb-8 text-center" style={{ backgroundColor: "#8BE9FD1a", borderColor: "#8BE9FD40" }}>
            <p className="font-semibold" style={{ fontSize: "16px" }}>Drop a file below to calculate its checksums.</p>
          </div>
        </div>
      </div>
