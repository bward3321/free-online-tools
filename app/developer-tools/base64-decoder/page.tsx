        ...
        <div className="max-w-[800px] mx-auto px-4 py-8 md:py-12">
          ...
          <article className="space-y-6 mb-10">
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              ...
              <p className="leading-relaxed" style={{ fontSize: "17px", lineHeight: "1.7", color: "var(--text-muted)" }}>Developers frequently need to decode Base64 when <strong style={{ color: "var(--text)" }}>debugging API responses</strong> that contain encoded payloads, <strong style={{ color: "var(--text)" }}>inspecting JWT tokens</strong> (the header and payload are Base64url-encoded JSON), reading <strong style={{ color: "var(--text)" }}>HTTP Basic Auth</strong> headers, extracting data from <strong style={{ color: "var(--text)" }}>email MIME attachments</strong>, or examining <strong style={{ color: "var(--text)" }}>webhook payloads</strong> from services like GitHub, Stripe, or AWS.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-[22px] sm:text-[28px] font-bold mb-3">Handling Decoding Errors</h2>
              <p className="leading-relaxed" style={{ fontSize: "17px", lineHeight: "1.7", color: "var(--text-muted)" }}>The most common Base64 decoding errors come from <strong style={{ color: "var(--text)" }}>URL-safe encoding</strong> (uses - and _ instead of + and /), <strong style={{ color: "var(--text)" }}>missing padding</strong> (some encoders strip trailing = characters), and <strong style={{ color: "var(--text)" }}>embedded whitespace</strong> (MIME-encoded data has line breaks every 76 characters). This tool handles all these cases automatically &mdash; toggle URL-safe mode if standard decoding fails, and whitespace is always stripped before decoding.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-[22px] sm:text-[28px] font-bold mb-3">Decoding JWT Tokens</h2>
              <p className="leading-relaxed" style={{ fontSize: "17px", lineHeight: "1.7", color: "var(--text-muted)" }}><a href="/developer-tools/json-formatter" style={{ color: "#8BE9FD" }} className="underline">JSON Formatter</a>.</p>
            </section>
          </article>
          <div className="rounded-xl border p-4 mb-8 text-center" style={{ backgroundColor: "#8BE9FD1a", borderColor: "#8BE9FD40" }}>
            <p className="font-semibold" style={{ fontSize: "17px" }}>Paste your Base64 string below &mdash; the decoder is already active.</p>
          </div>
        </div>
      </div>
