        ...
        <div className="max-w-[800px] mx-auto px-4 py-8 md:py-12">
          ...
          <article className="space-y-6 mb-10">
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">What Is HMAC?</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>HMAC (Hash-based Message Authentication Code) is defined in RFC 2104. It combines a <strong style={{ color: "var(--text)" }}>secret key</strong> with a <strong style={{ color: "var(--text)" }}>hash function</strong> to produce a message authentication code. While a regular hash only proves integrity (the data hasn&apos;t changed), HMAC also proves <strong style={{ color: "var(--text)" }}>authenticity</strong> &mdash; the message was created by someone who possesses the secret key. This makes HMAC essential for webhook verification and API authentication.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">Webhook Verification</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}><strong style={{ color: "var(--text)" }}>Stripe:</strong> Uses HMAC-SHA256. The signature is <code style={{ color: "#8BE9FD" }}>v1=&lt;hex&gt;</code> where the message is <code style={{ color: "#8BE9FD" }}>{"{timestamp}.{raw_body}"}</code>. <strong style={{ color: "var(--text)" }}>GitHub:</strong> Uses HMAC-SHA256 with the <code style={{ color: "#8BE9FD" }}>X-Hub-Signature-256</code> header in format <code style={{ color: "#8BE9FD" }}>sha256=&lt;hex&gt;</code>. <strong style={{ color: "var(--text)" }}>Shopify:</strong> Uses HMAC-SHA256, Base64-encoded in the <code style={{ color: "#8BE9FD" }}>X-Shopify-Hmac-Sha256</code> header.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">HMAC vs Regular Hashing</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>A regular hash like <code style={{ color: "#8BE9FD" }}>SHA256(message)</code> can be computed by anyone. An attacker could modify the message and recompute the hash. HMAC requires the secret key: <code style={{ color: "#8BE9FD" }}>HMAC-SHA256(key, message)</code>. Without the key, an attacker cannot forge a valid HMAC. This is why webhook services use HMAC &mdash; it proves the webhook came from the service, not an attacker.</p>
            </section>
          </article>
          <div className="rounded-xl border p-4 mb-8 text-center" style={{ backgroundColor: "#8BE9FD1a", borderColor: "#8BE9FD40" }}>
            <p className="font-semibold" style={{ fontSize: "16px" }}>Enter your secret key and message below to generate an HMAC signature.</p>
          </div>
        </div>
      </div>
