        ...
        <div className="max-w-[800px] mx-auto px-4 py-8 md:py-12">
          ...
          <article className="space-y-6 mb-10">
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">When You Need Beautified JSON</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>API responses and log files often contain minified JSON &mdash; a single line of text with no whitespace. Readable for machines, unreadable for humans. Beautifying adds indentation and line breaks so you can quickly scan the structure, find specific keys, and understand nested relationships.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">Formatting vs. Validating</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>Formatting changes how JSON <em>looks</em> (whitespace and indentation) without changing its data. Validation checks whether the JSON is <em>syntactically correct</em>. This tool does both: it formats valid JSON and shows detailed errors for invalid JSON.</p>
            </section>
          </article>
          <div className="rounded-xl border p-4 mb-8 text-center" style={{ backgroundColor: "#8BE9FD1a", borderColor: "#8BE9FD40" }}>
            <p className="font-semibold" style={{ fontSize: "16px" }}>Paste your JSON below to beautify it instantly.</p>
          </div>
        </div>
      </div>
