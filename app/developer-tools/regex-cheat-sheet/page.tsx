        ...
        <div className="max-w-[800px] mx-auto px-4 py-8 md:py-12">
          ...
          <article className="space-y-6 mb-10">
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">The Essential Regex Elements</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>Regular expressions are built from a small set of core elements. <strong style={{ color: "var(--text)" }}>Character classes</strong> like <code style={{ color: "#8BE9FD" }}>\d</code> (digits) and <code style={{ color: "#8BE9FD" }}>\w</code> (word characters) match categories of characters. <strong style={{ color: "var(--text)" }}>Anchors</strong> like <code style={{ color: "#8BE9FD" }}>^</code> and <code style={{ color: "#8BE9FD" }}>$</code> match positions, not characters. <strong style={{ color: "var(--text)" }}>Quantifiers</strong> like <code style={{ color: "#8BE9FD" }}>+</code> and <code style={{ color: "#8BE9FD" }}>*</code> control repetition. Combined, these build patterns from simple to complex.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">JavaScript-Specific Features</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>JavaScript&apos;s regex engine includes modern features: <strong style={{ color: "var(--text)" }}>named groups</strong> <code style={{ color: "#8BE9FD" }}>(?&lt;name&gt;...)</code> for readable captures, <strong style={{ color: "var(--text)" }}>lookbehind</strong> <code style={{ color: "#8BE9FD" }}>(?&lt;=...)</code> for matching based on preceding context, the <strong style={{ color: "var(--text)" }}>s flag</strong> making <code style={{ color: "#8BE9FD" }}>.</code> match newlines, and <strong style={{ color: "var(--text)" }}>Unicode property escapes</strong> <code style={{ color: "#8BE9FD" }}>\p{'{'}Script=Greek{'}'}</code> with the u flag.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">Interactive Learning</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>This isn&apos;t just a static reference &mdash; every example in the cheat sheet is <strong style={{ color: "var(--text)" }}>clickable</strong>. Click a row to load its pattern and sample text into the tester above. Modify the pattern or text to experiment. This makes it easy to learn regex by doing, not just reading.</p>
            </section>
          </article>
          <div className="rounded-xl border p-4 mb-8 text-center" style={{ backgroundColor: "#8BE9FD1a", borderColor: "#8BE9FD40" }}>
            <p className="font-semibold" style={{ fontSize: "16px" }}>Browse the cheat sheet below and click any example to test it.</p>
          </div>
        </div>
      </div>
