        ...
        <div className="max-w-[800px] mx-auto px-4 py-8 md:py-12">
          ...
          <article className="space-y-6 mb-10">
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">When PINs Are Appropriate</h2>
              <p className="leading-relaxed mb-3" style={{ fontSize: "16px", color: "var(--text-muted)" }}>PINs provide a limited level of security that relies on <strong style={{ color: "var(--text)" }}>rate-limiting</strong> &mdash; the device locks after a few failed attempts. A 4-digit PIN has only 10,000 possible combinations, which a computer could try exhaustively in under a second. But when a phone locks after 10 wrong guesses, an attacker gets 10 chances out of 10,000 &mdash; a 0.1% success rate.</p>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>This makes PINs appropriate for <strong style={{ color: "var(--text)" }}>physical devices</strong> (phones, ATMs, safes) where rate-limiting is enforced by hardware. They should never be used as passwords for online accounts, where an attacker may obtain the hashed PIN and crack it offline.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">Common PIN Mistakes</h2>
              ...
              <p className="mt-3 leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>A truly random PIN avoids all of these patterns. Every digit is independently selected with equal probability.</p>
            </section>
          </article>
          <div className="rounded-xl border p-4 mb-8 text-center" style={{ backgroundColor: "#0596691a", borderColor: "#05966940" }}>
            <p className="font-semibold" style={{ fontSize: "16px" }}>Generate your random PIN below &mdash; PIN mode is already selected.</p>
          </div>
        </div>
      </div>
