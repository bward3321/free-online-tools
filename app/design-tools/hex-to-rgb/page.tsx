        ...
        <div className="max-w-[800px] mx-auto px-4 py-8 md:py-12">
          ...
          <article className="space-y-6 mb-10">
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">How HEX Color Codes Work</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>HEX color codes use the base-16 (hexadecimal) number system to represent colors. A 6-digit hex code like <code style={{ color: "#059669" }}>#3B82F6</code> contains three pairs of digits: the first pair (<code style={{ color: "#059669" }}>3B</code>) represents red (59 in decimal), the middle pair (<code style={{ color: "#059669" }}>82</code>) represents green (130), and the last pair (<code style={{ color: "#059669" }}>F6</code>) represents blue (246). Each channel ranges from 00 (0) to FF (255).</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">The Conversion Formula</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}>Converting HEX to RGB is straightforward: split the 6-character hex string into three 2-character pairs, then convert each pair from hexadecimal to decimal. For example, <code style={{ color: "#059669" }}>#FF6347</code> (Tomato) becomes R: 255 (FF), G: 99 (63), B: 71 (47), giving <code style={{ color: "#059669" }}>rgb(255, 99, 71)</code>. Three-digit shorthand like <code style={{ color: "#059669" }}>#F63</code> doubles each digit first: FF6633.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-xl font-bold mb-3">Common HEX to RGB Conversions</h2>
              <p className="leading-relaxed" style={{ fontSize: "16px", color: "var(--text-muted)" }}><code style={{ color: "#059669" }}>#FFFFFF</code> = rgb(255, 255, 255) White. <code style={{ color: "#059669" }}>#000000</code> = rgb(0, 0, 0) Black. <code style={{ color: "#059669" }}>#FF0000</code> = rgb(255, 0, 0) Red. <code style={{ color: "#059669" }}>#00FF00</code> = rgb(0, 255, 0) Green. <code style={{ color: "#059669" }}>#0000FF</code> = rgb(0, 0, 255) Blue. <code style={{ color: "#059669" }}>#3B82F6</code> = rgb(59, 130, 246) Tailwind blue-500.</p>
            </section>
          </article>
          <div className="rounded-xl border p-4 mb-8 text-center" style={{ backgroundColor: "#0596691a", borderColor: "#05966940" }}>
            <p className="font-semibold" style={{ fontSize: "16px" }}>Type a HEX code below to convert it to RGB.</p>
          </div>
        </div>
      </div>
