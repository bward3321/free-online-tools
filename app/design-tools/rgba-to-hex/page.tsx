        ...
        <div className="max-w-[800px] mx-auto px-4 py-8 md:py-12">
          ...
          <article className="space-y-6 mb-10">
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-[22px] sm:text-[28px] font-bold mb-3">Understanding the Alpha Channel</h2>
              <p className="leading-relaxed" style={{ fontSize: "17px", lineHeight: "1.7", color: "var(--text-muted)" }}>The alpha channel controls <strong style={{ color: "var(--text)" }}>opacity</strong> &mdash; how transparent or opaque a color is. In RGBA, alpha is a decimal from 0 (fully transparent) to 1 (fully opaque). In 8-digit hex, alpha is the last two digits, ranging from 00 (transparent) to FF (opaque). The conversion: multiply the decimal alpha by 255 and convert to hexadecimal.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-[22px] sm:text-[28px] font-bold mb-3">Common Alpha Values</h2>
            ...
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-[22px] sm:text-[28px] font-bold mb-3">8-Digit Hex in CSS and Development</h2>
              <p className="leading-relaxed" style={{ fontSize: "17px", lineHeight: "1.7", color: "var(--text-muted)" }}>CSS supports 8-digit hex natively: <code style={{ color: "#059669" }}>background-color: #3B82F680;</code> gives blue at 50% opacity. In Android development, the format is <code style={{ color: "#059669" }}>#AARRGGBB</code> (alpha first). Some design tools and game engines use 8-digit hex for storing colors with transparency in a single compact value.</p>
            </section>
          </article>
          <div className="rounded-xl border p-4 mb-8 text-center" style={{ backgroundColor: "#0596691a", borderColor: "#05966940" }}>
            <p className="font-semibold" style={{ fontSize: "17px" }}>Adjust the RGBA alpha value below to convert.</p>
          </div>
        </div>
      </div>
