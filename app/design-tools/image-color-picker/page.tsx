        ...
        <div className="max-w-[800px] mx-auto px-4 py-8 md:py-12">
          ...
          <article className="space-y-6 mb-10">
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-[22px] sm:text-[28px] font-bold mb-3">Design Inspiration from Images</h2>
              <p className="leading-relaxed" style={{ fontSize: "17px", lineHeight: "1.7", color: "var(--text-muted)" }}>Some of the best color palettes come from photographs, artwork, and nature. Upload a sunset photo and extract its warm oranges and purples. Use a product photo to match exact brand colors. Sample colors from a competitor&apos;s website screenshot. This tool turns any image into a starting point for your color palette.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-[22px] sm:text-[28px] font-bold mb-3">How Color Extraction Works</h2>
              <p className="leading-relaxed" style={{ fontSize: "17px", lineHeight: "1.7", color: "var(--text-muted)" }}>The tool reads pixel data from your image using the HTML Canvas API. For dominant color extraction, it samples thousands of pixels, quantizes similar colors into groups, and returns the most frequent color groups. This identifies the image&apos;s key colors without processing every single pixel, keeping extraction fast even for large images.</p>
            </section>
            <section className="rounded-xl border p-5" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <h2 className="text-[22px] sm:text-[28px] font-bold mb-3">100% Private Processing</h2>
              <p className="leading-relaxed" style={{ fontSize: "17px", lineHeight: "1.7", color: "var(--text-muted)" }}>Your images <strong style={{ color: "var(--text)" }}>never leave your device</strong>. The browser reads the file locally, renders it to a canvas element, and extracts pixel data &mdash; all in JavaScript. No server upload, no API calls, no storage. This means you can safely use it with confidential design files, unreleased branding, and private photos.</p>
            </section>
          </article>
          <div className="rounded-xl border p-4 mb-8 text-center" style={{ backgroundColor: "#0596691a", borderColor: "#05966940" }}>
            <p className="font-semibold" style={{ fontSize: "17px" }}>Upload an image below to extract colors.</p>
          </div>
        </div>
      </div>
